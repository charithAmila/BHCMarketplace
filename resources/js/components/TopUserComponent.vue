<template>
	<div>
		<div class="seller-content">
			...
			<select id="user-type" class="" name="userType" @change="filterUser('user', $event)">
			    <option class="select-items" value="sell">Sellers</option>
			    <option class="select-items" value="buy">Buyers</option>
			</select>
			<strong>in</strong>
			<select id="filter-time" class="" name="filterTime" @change="filterUser('time', $event)">
		...
		<div class="filterList d-none d-md-block">
			...
			<div id="actual-top-user">
				<div v-for="(item, index) in userList" :key="index" class="topUserList">
					...
					<div class="user-info">
						<h6><a class="black-link" :href="item.profile_url">{{ item.display_name }}</a></h6>
		...
		<div class="filterListMobile d-sm-block d-md-none">
			<div v-for="(item, index) in userList" :key="index" class="filterItemMobile">
				<img class="filterImg" :src="asset_url+item.asset_url">
...
export default{
	...
	methods: {
		...
		fetchFilteredUser(){
			axios.get('/nft/user/filter/'+this.userType+'/'+this.filterTime).then((res) => {
