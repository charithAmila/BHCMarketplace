<nav class="navbar navbar-expand-lg navbar-light custom-navbar">
  ...
  <div class="right-nav d-none d-md-block default-nav">
    <a class="nav-item right-border {{ Request::segment(1) == 'nft' ? 'active' : ''}}" href="{{ route('marketplace') }}"><span>Marketplace</span></a>
    @if (Auth::check())
    <a class="nav-item right-border {{ Request::segment(1) == 'profile' ? 'active' : ''}}" href="{{ route('user.profile', Auth::user()->short_url != null ? Auth::user()->short_url : Auth::user()->wallet) }}">My NFT's</a>
     @endif
    <a class="nav-item right-border {{ Request::segment(1) == 'faq' ? 'active' : ''}}" href="{{ route('faq') }}">FAQ</a>
    <a class="nav-item right-border {{ Request::segment(1) == 'bhc' ? 'active' : ''}}" href="{{ route('about') }}">HPS</a>
    <a href="{{ route('create.collectible.choices') }}" class="btn nav-create">Create</a>

    @if (Auth::check())
    <a class="nav-item notif-btn" href="javascript:void(0)">
      <img src="{{ asset('images/logo2.png') }}" width="35" class="d-inline-block align-top" alt="" loading="lazy">
    </a>
    <a class="nav-item profile-btn" href="{{ route('user.profile', Auth::user()->short_url != null ? Auth::user()->short_url : Auth::user()->wallet) }}">
      @if(Auth::user()->display_photo == 'default.png')
      <img class="navbar-img-profile" src="{{ asset('/user/photo/'.Auth::user()->display_photo)}}" width="30" class="d-inline-block align-top" alt="" loading="lazy">
      @else
      <img class="navbar-img-profile" src="{{ asset('/storage/user/photo/'.Auth::user()->display_photo)}}" width="30" class="d-inline-block align-top" alt="" loading="lazy">
      @endif
    </a>
    @else
  ...
  <div class="profile-menu d-none">
    ...
    <div class="menu-options">
      <a href="{{ route('user.profile', Auth::user()->short_url != null ? Auth::user()->short_url : Auth::user()->wallet) }}">My account</a>
    ...
    <div class="modal-body">
      <div class="row">
        ...
        <div class="col-label col-7 col-md-7">
          ...
          <input type="file" id="user-photo" name='display_photo' class="user-check" accept="image/x-png,image/gif,image/jpeg"/>
          <input class="user-photo-btn btn-setPicture" type="button" value="Set picture" onclick="document.getElementById('user-photo').click();" />
      ...
      <div class="form-section">
        <form id="update-preferences" method="POST">
          ...
          <div class="form-divide">
            ...
            <input id="name-profile" class="modal-input preferences-check" type="text" name="name" placeholder="E.g. John Doe">
            <p class="preferences-error text-danger"></p>
            <span id="name-preferences" class="custom-error text-danger"></span>
          </div>
          <div class="form-divide">
            <label class="input-label">Description</label>
            <input id="description-profile" class="modal-input" type="text" name="description" placeholder="Spread some words about yourself">
            <p class="preferences-error text-danger"></p>
          ...
          <div class="form-divide">
            ...
            <input id="short_url-profile" class="modal-input preferences-check with-link" type="text" name="short_url" placeholder="Enter short url">
            <p class="preferences-error text-danger"></p>
